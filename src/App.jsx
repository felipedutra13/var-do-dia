...
function App() {
  ...
  useEffect(() => {
    ...
    if (gameMode === 'classic') {
      target = getDailyPlayer(mode);
      state = getGameState(mode);
      currentStats = getStats(mode);
    } else {
  ...
  const handleGuess = (player) => {
    ...
    const evaluation = evaluateGuess(player, targetPlayer, gameMode === 'career' ? 'mundial' : mode);
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex flex-col items-center py-8 px-4 sm:px-6">
      <header className="w-full max-w-2xl mb-6 flex flex-col gap-4">
        ...
        {/* Mode Selector — Carreira desabilitado temporariamente, remover o `false &&` para reativar */}
        {false && (
      ...
      <main className="w-full max-w-2xl flex-1 flex flex-col">
        {/* CareerTrajectory — desabilitado junto com o modo Carreira */}
        {false && gameMode === 'career' && (
          <CareerTrajectory 
        ...
        <div className="mb-8 w-full transition-all duration-300">
          {isGameOver ? (
            ...
            <div className="flex flex-col items-center">
              <SearchInput onSelect={handleGuess} disabled={isGameOver} mode={gameMode === 'career' ? 'mundial' : mode} />
        ...
        <div className="w-full flex-1 flex flex-col gap-1">
          {gameState.guesses.length > 0 && <GuessRow isHeader mode={mode} simpleMode={gameMode === 'career'} />}
          
          {gameState.guesses.map((guess, index) => (
            <GuessRow key={index} guess={guess} mode={mode} simpleMode={gameMode === 'career'} />
          ))}
